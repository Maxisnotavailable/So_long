...
/*   Updated: 2023/02/26 12:31:26 by molla            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	check_path(t_map *map, char **map_tmp)
{
	...
	{
		map->j = -1;
		while (map_tmp[map->i][++(map->j)])
			find_idx(map, map_tmp);
	}
	map->j = flood_fill(map_tmp, map_size, map->pos_idx.y, map->pos_idx.x);
	if (map->j != map->nb_item)
		return (write(2, "Error\nImpossible path\n", 22), 1);
	return (0);
}
